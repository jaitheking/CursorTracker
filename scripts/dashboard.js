...
function initializeSegmentTabs() {
...
function calculatePerformanceAnalytics() {
    ...
    runningLogs.forEach(log => {
        ...
        if (distMatch) {
            ...
            if (log.date.startsWith(targetMonthPrefix))
                monthlyMileage += dist;
            if (log.date.startsWith(targetYearStr))
                yearlyMileage += dist;
            if (dist > maxDistance)
                maxDistance = dist; // Absolute record check
            if (paceMatch) {
    ...
    periodGymLogs.forEach(log => {
        ...
        if (weightMatch)
            maxWeight = Math.max(maxWeight, parseFloat(weightMatch[1]));
        if (muscleMatch)
            maxMuscle = Math.max(maxMuscle, parseFloat(muscleMatch[1]));
        if (focusMatch) {
